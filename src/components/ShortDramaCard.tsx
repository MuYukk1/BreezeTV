...
function ShortDramaCard({
  ...
  return (
    <div
      className={`group relative ${className} transition-all duration-300 ease-in-out hover:scale-[1.05] hover:z-[500] hover:drop-shadow-2xl`}
    >
      <Link
        href={`/play?title=${encodeURIComponent(drama.name)}&shortdrama_id=${
          drama.id
        }`}
        className='block'
      >
